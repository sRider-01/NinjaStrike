...
void PlayerKunai::Update(float delta_time)
{
	...
	//クナイが画面外に出たら
	if (screen_x < -kMargin || screen_x >(screen_width + kMargin))
	{
		//発射済みフラグをFalseにする
		EndKunai();
		return;
...
/// クナイ発射処理
/// </summary>
void PlayerKunai::Fire(float pos_x, float pos_y, bool is_facing_left)
{
...
/// 当たり判定円を作る
/// </summary>
CollisionBase::CircleF PlayerKunai::MakeHitCircle() const
{
...
/// </summary>
bool PlayerKunai::TryGetHitCircle(CollisionBase::CircleF& out_circle)
{
	//発射されていないなら当たり判定は無し
	if (!kunai_info.is_fired)
	{
